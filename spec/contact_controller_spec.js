...
describe("ContactController", () => {
  ...
  describe("#addContact()", () => {
    it("should add a single contact into the book", done => {
      this.book
        .addContact("Alice", "001-101-1010", "alicethehousewife@gmail.com")
        .then(contact => {
      ...
      it("should return null when contact is not found", done => {
        this.book.addContact(...zelda).then(() => {
          this.book
            .getContacts()
            .then(contacts => {
              expect(this.book.iterativeSearch(contacts, "Alloy Rodriguez"));
              done();
            })
            .catch(err => {
              console.log(err);
        ...
        this.book.addContact(...zelda).then(() => {
          this.book
            ...
            .then(contacts => {
              expect(
                this.book
                  .binarySearch(sort(contacts), "Alloy Rodriguez")
                  .toBeNull()
              );
              done();
            })
